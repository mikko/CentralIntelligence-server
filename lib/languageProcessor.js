...
const selectClient = (tokenizedMessage, actionConfigs) => {
    ...
    let exactMatch;
    actionConfigs.forEach(client => {
        Object.keys(client.actions).forEach(actionKey => {
            const action = client.actions[actionKey];
            const exactPhrase = action.exactPhrase || '';
            console.log('Checking for exact phrase', exactPhrase);
            if (exactPhrase.toLowerCase() === tokenizedMessage.originalMessage.toLowerCase()) {
                console.log('Exact phrase used');
                exactMatch = {
                    client: client.name,
                    action: actionKey
                };
